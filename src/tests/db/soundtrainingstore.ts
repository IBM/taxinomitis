...
describe('DB store - sound training', () => {
    ...
    function isTrainingData(item: Objects.SoundTraining): boolean {
        return item &&
               ...
               item.label.length > 0 &&
               Array.isArray(item.audiodata) &&
               item.audiodata.length > 0;
    }

    async function createTestData(projectid: string, numItems: number, numLabels: number) {
        const labels = [];
    ...
    describe('storeSoundTraining', () => {
        ...
        it('should limit maximum training data length', async () => {
            const projectid = uuid();
            const data = createTraining(dbobjects.MAX_AUDIO_POINTS * 2);
            const label = uuid();
